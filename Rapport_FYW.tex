...
\begin{document}
  ...
  Cet article très complêt explique l'ensemble de sa solution et pourrait être implémenté dans le cadre de ce stage, cependant aucun code
  source sur lequel nous pourrions nous baser n'est fournis alors son implémentation prendrait un temps considérable. Aussi, les auteurs 
  ...
  leur travail.
  Leur méthode de localisation intérieur utilisant VSLAM nécéssite de créer une carte de l'environnement avant l'utilisation du module, cependant
  ...
  Computing}
  /!\ Relire pour completer les acronymes et explications manquantes.

  La solution CDSFusion se sert d'images RGB comme l'article précédent ainsi qu'un capteur IMU comme paramètres d'entrée et est composé de 
  ...
  expérimentaux montrent que les features FAST augmente la rapidité du système d'une manière plus conséquente que les features Shi-Tomas 
  ...
  % TODO: Heu, c'est quoi le problème des fenêtres glissantes ? Se renseigner.
  \underline{Back-End:} est utilsié pour fusionner les mesures traitées afin d'obtenir l'estimation de la position. Une optimisation non-linéaire
  ...
  basant sur des calculs précédemment éffectués. Cela se base sur la bibliothèque DBoW2 qui est à l'état de l'art de la reconnaissance d'endroits 
  ...
  La segmentation sémantique resulte d'images RGB en entrée qui sont acquises en temps réel en utilisant le module de segmentation PSPNet.
  L'implémentation originale de PSPNet est faite avec PyTorch.
  Le module de segmentation sémantique traite chaque image RGB and retourne des vecteurs indiquant la probabilité d'appartenance à une classe
  ...
  L'image segmentée finale est composée des pixels colorés et est transmis au module de reconstruction 3D.

  \paragraph{Le module de reconstruction 3D}

  Le nuage sémantique local est géneré en utilisant une image sémantique (générée par la module précédant) et une carte de profondeur. 
  Ce nuage local servira à produire un nuage global une fois qu'il sera combiné avec les estimation de position de la caméra depuis le module VIO.
  ...
  A chaque image importante (Keyframe), la carte de profondeur courante est transformée en nuage de points 3D, ensuite les options Bovblox et FAST
